...
func block_player_control() -> void:
	# Método preferido: usar control directo
	if context and context.get_player():
		context.get_player().block_controls()
	else:
		# Fallback: usar SignalManager (compatibilidad temporal)
		SignalManager.player_control_blocked.emit()

func unblock_player_control() -> void:
	# Método preferido: usar control directo
	if context and context.get_player():
		context.get_player().unblock_controls()
	else:
		# Fallback: usar SignalManager (compatibilidad temporal)
