...
func _ready():
	...
	SignalManager.battle_transition_requested.connect(_on_battle_transition_requested)
	SignalManager.battle_reveal_requested.connect(_on_battle_reveal_requested)
...
func play_battle_transition(texture_path: String, duration: float = 1.0) -> void:
	...
	SignalManager.battle_transition_finished.emit()
...
func reveal_battle(duration: float = 0.4) -> void:
	...
	SignalManager.battle_reveal_finished.emit()

## Maneja las solicitudes de transición de batalla desde el SignalManager
...
## Maneja las solicitudes de revelar batalla desde el SignalManager
