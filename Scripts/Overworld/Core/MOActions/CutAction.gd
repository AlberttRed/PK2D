...
func can_use(_player: Node, target: Node) -> bool:
	...
	if not target:
		print("CutAction: Target es nulo")
		return false
	...
	if not pokemon_with_cut:
		print("CutAction: Ningún Pokémon en el equipo conoce CORTE")
		return false

	print("CutAction: %s conoce CORTE" % pokemon_with_cut.get_display_name())
	...
	#     print("CutAction: No tiene la medalla CASCADE necesaria")
	#     return false
...
func execute(_player: Node, target: Node, context: Node) -> Dictionary:
	print("CutAction: Ejecutando flujo de CORTE sobre '%s'" % target.name)
	...
	if requires_confirmation:
		...
		if choice != 0:  # No o cancelado
			print("CutAction: Jugador canceló el uso de CORTE")
	...
	# 4. Retornar éxito
	print("CutAction: CORTE completado exitosamente por %s" % pokemon_name)
