...
static func get_candidate_move_id(pokemon: BattlePokemon) -> int:
	...
	if move_id <= 0 or move_id in UNENCOREABLE_MOVE_IDS:
		return 0
	if find_move_index_by_id(pokemon, move_id) < 0:
		return 0
	var move := get_move_by_id(pokemon, move_id)
	if move == null or move.get_pp() <= 0:
		return 0
	return move_id


static func find_move_index_by_id(pokemon: BattlePokemon, move_id: int) -> int:
	if pokemon == null or move_id <= 0:
		return -1
...
static func get_move_by_id(pokemon: BattlePokemon, move_id: int) -> BattleMove:
	var idx := find_move_index_by_id(pokemon, move_id)
	if idx < 0:
		return null
	return pokemon.get_available_moves()[idx]
...
func apply_selectable_moves_filter(pokemon: BattlePokemon, filter: MoveSelectionFilter) -> void:
	if not is_active() or pokemon != target:
		return
	var locked_index := find_move_index_by_id(pokemon, _locked_move_id)
...
func _is_locked_move_usable(pokemon: BattlePokemon) -> bool:
	var move := get_move_by_id(pokemon, _locked_move_id)
	if move == null or move.get_pp() <= 0:
		return false
	if TauntAilmentEffect.blocks_status_move(pokemon, move):
		return false
	return true
