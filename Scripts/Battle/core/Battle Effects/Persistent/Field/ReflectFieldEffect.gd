...
func visualize_phase(_pokemon, ui: BattleUI, phase: Phases) -> void:
    if phase == Phases.ON_ENTRY:
        await ui.show_start_effect_message(MessageFamily.Values.FIELD_EFFECT, null, source.id)
    elif phase == Phases.ON_END_BATTLE_TURN and has_finished():
        await ui.show_end_effect_message(MessageFamily.Values.FIELD_EFFECT, null, source.id)
