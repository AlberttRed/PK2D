...
## Script para renombrar archivos de Pokémon a formato "001 - Nombre.tres"
...
func _run() -> void:
	...
	if dir == null:
		push_error("No se pudo abrir directorio: %s" % pokemon_dir)
		return
	...
	while file_name != "":
		...
		if resource.has_method("get"):
			...
			# Si no, usar internal_name
				...
				if internal_name_value != null:
					...
					if internal_name != "":
						pokemon_name = internal_name.capitalize()
		...
		pokemon_name = _sanitize_filename(pokemon_name)

		# Construir nuevo nombre
		var new_name := "%03d - %s.tres" % [id, pokemon_name]
		var new_path := pokemon_dir + "/" + new_name
