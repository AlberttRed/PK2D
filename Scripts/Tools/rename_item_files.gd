...
## Script para renombrar archivos de Items a formato "001 - Nombre.tres"
...
func _run() -> void:
	...
	if dir == null:
		push_error("No se pudo abrir directorio: %s" % items_dir)
		return
	...
	while file_name != "":
		...
		if resource.has_method("get"):
			...
			if display_name_value != null and str(display_name_value) != "":
				item_name = str(display_name_value)
			# Si no, usar internal_name
				...
				if internal_name_value != null:
					...
					if internal_name != "":
						item_name = internal_name.capitalize()
		...
		item_name = _sanitize_filename(item_name)

		# Construir nuevo nombre
		var new_name := "%03d - %s.tres" % [id, item_name]
		var new_path := items_dir + "/" + new_name
